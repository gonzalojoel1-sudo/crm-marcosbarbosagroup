"""Limpieza temporal del E2E del embudo: borra negocios/leads/organizaciones de prueba."""
import sys
...
DEALS = ["CRM-DEAL-2026-00019", "CRM-DEAL-2026-00020"]
LEADS = ["CRM-LEAD-2026-00013"]
ORGS = ["ZZ Prueba Embudo", "Zz Org"]
...
for l in LEADS:
    if frappe.db.exists("CRM Lead", l):
        frappe.delete_doc("CRM Lead", l, force=True, ignore_permissions=True)

for o in ORGS:
    for name in frappe.get_all(
        "CRM Organization", filters={"organization_name": o}, pluck="name"
    ):
...
print(
    "orgs:",
    [r["organization_name"] for r in frappe.get_all("CRM Organization", fields=["organization_name"])],
)
